...
class Balloon:
    ...
    def move(self):
        ...
        if self.y <= self.radius or self.y >= HEIGHT - self.radius * 2:
            self.direction *= -1
...
balloons_popped = 0
game_over = False
game_time = 30 * 1000  # 1 minute in milliseconds
...
while not game_over:
    ...
    for bullet in bullets[:]:
        ...
        if (balloon.x - balloon.radius < bullet.x < balloon.x + balloon.radius) and (
                balloon.y - balloon.radius < bullet.y < balloon.y + balloon.radius):
            bullets.remove(bullet)
            balloons_popped += 1
            balloon = Balloon(100, random.randint(25, HEIGHT - 50), 25)

    # Display countdown
    countdown_text = font.render(f"Time left: {remaining_time // 1000}s", True, FONT_COLOR)
    screen.blit(countdown_text, (10, 10))

    # Display balloons popped
    balloons_popped_text = font.render(f"Balloons popped: {balloons_popped}", True, FONT_COLOR)
    screen.blit(balloons_popped_text, (10, 50))

    pygame.display.flip()

# Prepare Game Over text and final score
game_over_text = font.render("Game Over!", True, FONT_COLOR)
final_score_text = font.render(f"Balloons popped: {balloons_popped}, Missed shots: {missed_shots}", True, FONT_COLOR)
